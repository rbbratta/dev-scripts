...
function generate_ocp_host_manifest() {
    ...
    worker_index=0
    jq --raw-output '.[] | .name + " " + .ports[0].address + " " + .driver_info.username + " " + .driver_info.password + " " + .driver_info.address + " " + .properties.cpu_arch' $host_input \
       | while read name mac username password address architecture; do

        encoded_username=$(echo -n "$username" | base64)
        encoded_password=$(echo -n "$password" | base64)
        # Lab default: BMC TLS certs (e.g. iDRAC SAN mismatch) are not verified.
